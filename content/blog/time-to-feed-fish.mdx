...
The fish feeder also comes with an underwater temperature sensor and water-lever meter to track the current underwater temperature as well as the water level to detect a possible water leak in advance. In the backend, the Raspberry Pi Pico is connected to a [Google Firebase Firestore Database](https://github.com/Joserra13/automated-aquarium/blob/main/docs/firebase.md) via HTTP requests and updates the values in periods of 5 seconds. Then, those values are fetched from the frontend and are displayed in the [stream](https://automated-aquarium.com/stream) page.
